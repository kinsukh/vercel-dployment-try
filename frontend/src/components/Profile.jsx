...
function Profile({ user }) {
  ...
  const handleUpdatePassword = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:5500/users/update-password', {
        userId: user._id,
  ...
  const handleDeleteProfile = async () => {
    ...
    try {
      const response = await axios.delete(`http://localhost:5500/users/delete-user/${user._id}`);
