...
function PlaylistDisplay(props) {
  ...
  useEffect(() => {
    const fetchUserPlaylists = async () => {
      try {
        if (user) {
          const response = await axios.get(`http://localhost:5500/playlists/userPlaylists`, {
            params: {
              userId: user.email,
            },
          });
    ...
    if (playlistName) {
      if (isPlaylistNameExists(playlistName)) {
        ...
        try {
          ...
          if (user) {
            const playlistResponse = await axios.post('http://localhost:5500/playlists/addPlaylist', {
              userId: user.email,
              playlistName,
            ...
            await axios.post('http://localhost:5500/playlists/addSong', {
  ...
  const onAddSong = ((playlistId) => {
    axios.post('http://localhost:5500/playlists/addSong', {
